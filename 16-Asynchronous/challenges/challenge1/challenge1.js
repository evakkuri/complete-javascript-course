...
5. This API allows you to make only 3 requests per second. If you reload fast,
  ...
  Remember, fetch() does NOT reject the promise in this case. So create an
...
const whereAmI = function (lat, lon) {
  ...
  fetch(queryUrl)
    .then((response) => {
      ...
      if (!response.ok) {
        switch (response.status) {
          case 403:
            throw new Error(
              `Could not retrieve due to throttling (${response.statusText}, ${response.status})`
            );
          default:
            throw new Error(
              `Could not retrieve due to error: ${response.statusText}`
            );
        }
      }
    ...
    .then((data) => {
      ...
      const countryDataUrl = `${countryUrl}/name/${data.country.toLowerCase()}`;
